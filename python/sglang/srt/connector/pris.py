...
class PrisConnector(BaseKVConnector):
    ...
    def __init__(self, url: str, **kwargs):
        ...
        self.connection = pris.PrisClient(
            parsed_url.hostname, parsed_url.port, password
        )
        self.model_name = parsed_url.path.lstrip("/")

    def _get(self, key: str, value: torch.Tensor) -> int:
        try:
            reg_buf = self.connection.reg_memory(
                value.data_ptr(), value.element_size() * value.numel()
            )
            if reg_buf == 0:
                return -1

            sgl = pris.SGL(
                value.data_ptr(), value.element_size() * value.numel(), reg_buf
            )
            return self.connection.get(key, sgl, value.element_size() * value.numel())
        finally:
            self.connection.dereg_memory(reg_buf)

    def get(self, key: str) -> Optional[torch.Tensor]:
        meta_key = key + "/metadata"
        meta = torch.empty(METADATA_LENGTH, dtype=METADATA_DTYPE, device="cpu")
        ret = self._get(meta_key, meta)
        if ret != 0:
            logger.error("Failed to get metadata for key %s, ret = %d", meta_key, ret)
            return None

        obj = _parse_metadata(meta, device=self.device)
        ret = self._get(key, obj)
        if ret != 0:
    ...
    def _set(self, key: str, value: torch.Tensor) -> int:
        try:
            reg_buf = self.connection.reg_memory(
                value.data_ptr(), value.element_size() * value.numel()
            )
            ...
            return self.connection.set(key, sgl)
